...
bool IRWrapper::is_in_subquery(IR* cur_stmt, IR* check_node,
    ...
    while (1) {
        if (cur_iter == NULL) { // Iter to the parent node. This is Not a subquery. 
            ...
            cur_iter->get_parent()->get_ir_type() != kSelectStmt
        ) {
            return true;
        }
        else if (
...
IR* IRWrapper::get_first_stmtmulti_from_root() {
    ...
    if (ir_root->get_left()->get_ir_type() != kStmtlist) {
        cerr << "Error: In ir_wrapper:get_stmtmulti_IR_vec, cannot find the kStmtlist " \
...
IR* IRWrapper::get_first_stmt_from_root() {
    ...
    if (first_stmtmulti == NULL) {
        cerr << "DEBUG: In IRWrapper::get_first_stmt_from_root, this->get_first_stmtmulti_from_root() returns NULL. \n\n\n";
        return NULL;
    }
...
vector<IR*> IRWrapper::get_stmtmulti_IR_vec(){
    ...
    /* Ignore the last kStmtlist, the last one is just a semicolon. 
     * Ignore this line if we are not using the built-in Postgres parser. 
     */
...
bool IRWrapper::append_stmt_at_idx(IR* app_IR_node, int idx) { // Please provide with IR* (Specific_Statement*) type, do not provide IR*(StatementList*) type.
    ...
    if (idx <= stmt_list_v.size()) {
        ...
        auto new_res = new IR(kStmtlist, OPMID(";"), NULL, app_IR_node);
...
bool IRWrapper::is_exist_UNION_SELECT(IR* cur_stmt) {
    ...
    vector<IR*> v_simple_select = get_ir_node_in_stmt_with_type(cur_stmt, kCombineClause, false, false, false);
...
bool IRWrapper::is_exist_INTERSECT_SELECT(IR* cur_stmt) {
    ...
    vector<IR*> v_simple_select = get_ir_node_in_stmt_with_type(cur_stmt, kCombineClause, false, false, false);
    for (IR* cur_simple_select : v_simple_select){
        if (cur_simple_select->get_prefix() == "INTERSECT") {
            return true;
        }
    }
    return false;
...
bool IRWrapper::is_exist_EXCEPT_SELECT(IR* cur_stmt) {
    ...
    vector<IR*> v_simple_select = get_ir_node_in_stmt_with_type(cur_stmt, kCombineClause, false, false, false);
    for (IR* cur_simple_select : v_simple_select){
        if (cur_simple_select->get_prefix() == "EXCEPT") {
            return true;
        }
    }
    return false;
...
IR* IRWrapper::get_stmt_ir_from_stmtmulti(IR* cur_stmtmulti){
    ...
    if (cur_stmtmulti->get_ir_type() != kStmtlist) {
        cerr << "Error: In IRWrapper::get_stmt_ir_from_stmtmulti(), not getting type kStmtlist. \n";
        return NULL;
    }
    ...
    //     return cur_stmtmulti->get_left();
        ...
        // cerr << "Error: Cannot find specific stmt from kStmtlist. \n";
        return NULL;
    }
}
