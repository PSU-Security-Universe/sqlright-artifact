...
public:
  ...
  vector<IR *> split_to_substmt(IR *cur_stmt, map<IR **, IR *> &m_save,
  ...
  */
  ...
  bool fix(IR*);
  bool fix_one(IR*, map<int, map<DATATYPE, vector<IR*>>> &scope_library);
  bool fill_stmt_graph(map<IR*, vector<IR*>> &graph);
  bool fill_stmt_graph_one(map<IR*, vector<IR*>> &graph, IR*);
  void clear_scope_library(bool clear_define);
  bool fill_one_pair(IR*, IR*);
  vector<IR *> split_to_stmt(IR * root, map<IR**, IR*> &m_save, set<IRTYPE> &split_set);
