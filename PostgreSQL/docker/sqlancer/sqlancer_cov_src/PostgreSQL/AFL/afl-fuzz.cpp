...
public:
  ...
  POSTGRES_OUTPUT execute(string cmd)
  {
    ...
    for (string& cur_cmd : cmd_vec) {
      ...
      while ( !is_timeout  &&  (res = PQgetResult(conn)) && res != NULL ) {
        ...
        //       result.outputs += string(PQgetvalue(res, 0, j)) + " "; 
        //       time(&process_timer);
  ...
  void reset_database()
  {
    ...
    if (PQstatus(conn) != CONNECTION_OK) {
      disconnect(conn);
      cerr << "Error: Reset database failed. \n\n\n";
      return;
    }
...
//       u8 new_fault;
//       new_fault = run_target(argv, hang_tmout, query_str);
...
static void maybe_update_plot_file(double bitmap_cvg, double eps)
{
  ...
  if (cur_SQLancer_timestamp.size() > 0 && cur_SQLancer_timestamp[0] != '[') {
    cur_SQLancer_timestamp = "";
  }
...
EXP_ST void setup_dirs_fds(void)
{
  ...
  fprintf(plot_file, "unix_time,cycles_done,cur_path,paths_total,"
                     ...
                     "mutate_failed_per,num_valid,num_parse,num_mutate_all,num_reparse,num_append,num_validate,sqlancer_timestamp\n"
                     );
...
int main(int argc, char **argv)
{
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QDc:lO:P:I:")) > 0)
  ...
  do {
    ...
    if ( !filesystem::exists(sqlancer_query_path)) {
