...
class Bisect:
    ...
    def _check_query_exec_correctness_under_commitID(queries_l, commit_ID: str, oracle):
        INSTALL_DEST_DIR = VerCon.setup_POSTGRE_with_commit(hexsha=commit_ID)

        if INSTALL_DEST_DIR == "":
            # Failed to compile commit.
            log_out_line("[-] Commit failed to compile: {}\n".format(commit_ID))
            return RESULT.FAIL_TO_COMPILE, None, None  
        else:
            log_out_line("[+] Success compiled: {}\n".format(commit_ID))

        all_res_str_l = []
        for queries in queries_l:
            all_res_str, res = Executor.execute_queries(
                queries=queries, postgre_install_dir=INSTALL_DEST_DIR, oracle=oracle
            )
    ...
    def bi_secting_commits(
        ...
        all_tags_sorted = vercon.all_tags_sorted
        ...
        for current_tag in all_tags_sorted.keys():  # From the latest tag to the earliest tag.
            ...
            while not is_successfully_executed:
                ...
                if rn_correctness == RESULT.PASS:  # Execution result is correct.
                    log_out_line(f"[*] RESULT.PASS: {current_commit_index} {current_commit_str}")
                    ...
                    log_out_line(f"[*] RESULT.FAIL: {current_commit_index} {current_commit_str}")
                    ...
                    log_out_line(f"[*] RESULT.ALL_ERROR: {current_commit_index} {current_commit_str}")
                    older_commit_str = current_commit_str
                    is_successfully_executed = True
                    is_commit_found = True
                    ...
                    log_out_line(f"[*] RESULT.FAIL_TO_COMPILE: {current_commit_index} {current_commit_str}")
                    ...
                    log_out_line(f"[*] RESULT.ELSE: {current_commit_index} {current_commit_str}")
                    newer_commit_str = current_commit_str
                    is_successfully_executed = False
                    is_commit_found = False
                    break
        ...
        while not is_buggy_commit_found:
            ...
            while not is_successfully_executed:
                ...
                if rn_correctness == RESULT.PASS:  # The correct version.
                    ...
                    log_out_line(f"[*] RESULT.ERROR: {commit_ID}")
                    older_commit_index = tmp_commit_index
                    is_successfully_executed = True
                    is_error_returned_from_exec = True
                    break
                    ...
                    log_out_line(f"[*] RESULT.ELSE: {commit_ID}")
                    newer_commit_index = tmp_commit_index
                    is_successfully_executed = False
                    is_error_returned_from_exec = True
                    break
