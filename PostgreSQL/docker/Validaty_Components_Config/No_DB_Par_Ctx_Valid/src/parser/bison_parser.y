...
%token TEMP UNLOGGED SECOND WINDOW NOTHING HAVING COUNT COALESCE SUM
%token DOUBLE_COLON
...
select_clause:
	SELECT opt_all_or_distinct select_target opt_cast opt_alias opt_from_clause opt_alias opt_where_clause opt_group_clause opt_window_clause  {
		$$ = new SelectClause();
		$$->case_idx_ = CASE0;
