...
vector<IR*> SQL_TLP::post_fix_transform_select_stmt(IR* cur_stmt, unsigned multi_run_id){
  ...
  vector<IR*> v_select_target = ir_wrapper.get_ir_node_in_stmt_with_type(cur_stmt, kSelectTarget, false);
  if (v_select_target.size() == 0 || !(v_select_target.front()->get_left()) ) {
    return trans_IR_vec;
  }
  IR* target_expr_list_ir = v_select_target.front()->get_left();
  if (target_expr_list_ir->get_right()) {
    /* Has multiple kExprList. Ignore. */
    return trans_IR_vec;
  }
...
VALID_STMT_TYPE_TLP SQL_TLP::get_stmt_TLP_type (IR* cur_stmt) {
  ...
  vector<IR*> v_distinct_clause = ir_wrapper.get_ir_node_in_stmt_with_type(cur_stmt, kOptAllOrDistinct, false);
  if (v_distinct_clause.size() > 0) {
    for (IR* distinct_clause : v_distinct_clause) {
      if (distinct_clause->op_ != NULL &&
          distinct_clause->get_prefix() &&
          !strcmp(distinct_clause->get_prefix(), "DISTINCT") 
        ) {
  ...
  for (IR* count_func_ir : count_func_vec){
    ...
    if (
      ir_wrapper.is_ir_in(count_func_ir, kSelectTarget)
    ) {
      IR* iden_ir = count_func_ir;
      if (!iden_ir) continue;
...
IR* SQL_TLP::transform_non_aggr(IR* cur_stmt, bool is_UNION_ALL, VALID_STMT_TYPE_TLP tlp_type) {
  ...
  /* Modify the second part of TLP. Put the Whereclause into (Not (kWhereClause)) */
...
//   vector<IR*> v_aggr_func_ir = ir_wrapper.get_ir_node_in_stmt_with_type(cur_stmt, kFuncExpr, false);
