...
def plot_sql_mapsize(file_dir, markevery, line_style, is_downsampling = True):
    ...
    for cur_file_name in os.listdir(file_dir):
        ...
        if last_idx < 10:
            print("Warning: The file:%s has too few entries. Skipped. " % (cur_file_name))
            continue
...
def plot_sqlancer_default_correct_rate(markevery=30, line_style = 2, is_downsampling=True):
    time_avg = []
    corr_rate_avg = []
    for i in np.arange(0, 72.2, 0.2):
        time_avg.append(i)
...
def plot_sqlancer_corr_over_time(file_dir, markevery=2600, line_style = 2, is_downsampling=True):
    # # For SQLancer
    all_time_delta = []
    all_corr_rate = []
    time_avg = []
...
def plot_sqlancer_corr_over_time_2(file_name, markevery, line_style, is_downsampling = True):
    ...
    for i in [0]:
        ...
        curr_rate_list = []
        for cur_curr_rate in file['total_execs']:
            cur_curr_rate = int(cur_curr_rate)
            curr_rate_list.append(cur_curr_rate * 100)
...
def plot_sql_corr_over_time(file_dir, markevery, line_style, is_downsampling = True):
    ...
    for i in [0]:
        ...
        curr_rate_list = []
        for cur_curr_rate in file['total_good_stmt']:
            cur_curr_rate = int(cur_curr_rate)
            curr_rate_list.append(cur_curr_rate)
    ...
    corr_rate_avg = corr_rate_avg[10:]
