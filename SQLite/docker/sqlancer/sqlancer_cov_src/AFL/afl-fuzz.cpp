...
string cur_SQLancer_timestamp = "";
string sqlancer_query_path = "";
...
EXP_ST void init_forkserver(char **argv) {
  ...
  if (!forksrv_pid) {
    ...
    char * argv_tmp[] = {"sqlite3", "test.db", NULL};
    execv(target_path, argv_tmp); // Used for start up sqlite3 ???
...
u8 execute_cmd_string(string cmd_string, vector<int> &explain_diff_id,
  ...
  // cerr << "Running with cmd_string: " << cmd_string << "\n\n\n";
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  fprintf(plot_file,
          "%llu, %llu, %u, %u, %u, %u, %0.02f%%, %llu, %llu, %u, %0.02f, %u, %u, %u, %0.02f%%, %u, %u, %u, %u, %u, %u, %u, %u, %0.02f%%, %u, %u, %s, %u\n",
          ...
          bug_output_id, queued_with_cov, cur_SQLancer_timestamp.c_str(), total_execs
          ); /* ignore errors */
  fflush(plot_file);
}
...
static void show_stats(void) {
  ...
  if (cur_ms - last_plot_ms > PLOT_UPDATE_SEC * 10) {
...
static u8 fuzz_one(char **argv, fstream& input_fd) {
  ...
  while (true) {
    ...
    /* Read 200 lines from input files. */
    string input = "";
    for (int i = 0; i < 200; i++){
      string tmp_input = "";
      if(!std::getline(input_fd, tmp_input)){
        cerr << "Reaching the end of file. " << endl;
        is_file_finished = true;
        break;
      }
      vector<string> res = string_splitter(tmp_input, "]");
      if (res.size() < 2) continue;
      cur_SQLancer_timestamp = res[0];
      tmp_input = "";
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QDFc:EO:I:")) > 0)
  ...
  do {
    ...
    for (const auto & entry : std::filesystem::directory_iterator(sqlancer_query_path)){
      ...
      string tempfile = entry.path();
      /* Monitor the creation of databaseXX.log files. */
      if (tempfile.find("-cur") != string::npos)
        continue;
      
      string curfile = tempfile.insert(tempfile.find(".log"), "-cur");
      if (!std::filesystem::exists(curfile)) 
        continue;

      std::cout << curfile << std::endl;
      input_file_vec.push_back(curfile);
    }

    if (input_file_vec.size() == 0) {
      cout << "not found new current file." << endl;
      // Sleep 20 sec.
      sleep(20);
    }

    // vector<string> input_file_vec = {"/data/liusong/sqlancer/logs/sqlite3/database0-cur.log"};
    for(const string&input_fd_path : input_file_vec ) {
      if (stop_soon){
        break;
      }
      remove("./test.db");
      input_fd.open(input_fd_path, ios::in);
      ...
      skipped_fuzz = fuzz_one(use_argv, input_fd);
      input_fd.close();
      show_stats();

    }

    // Remove current files generated by SQLancer.
    for(string cur_path : input_file_vec ) {
      std::filesystem::remove(cur_path);
    }
