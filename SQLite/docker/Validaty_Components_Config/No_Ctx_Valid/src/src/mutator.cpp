...
void Mutator::add_to_library(IR* ir){
        ...
        all_saved_ir.push_back(ir_copy);
        ...
        void Mutator::add_to_library_core(IR* ir){
            ...
            string Mutator::fix(IR * root){
                ...
                if(type_ == kFilePath || type_ == kOptOrderType
                   || type_ == kColumnType || type_ == kSetOperator || type_ == kOptJoinType
                ...
                if(type_ == kNumericLiteral || type_ == kconst_float) return std::to_string(float(value_libary[get_rand_int(value_libary.size())]) + 0.1);
		...
		static int counter = 0;
                ...
                if(type_ == kOptOrderType || type_ == kNullLiteral || type_ == kColumnType || type_ == kSetOperator || type_ == kOptJoinType || type_ == kOptDistinct) return str_val_;
                ...
                if(type_ == kNumericLiteral || type_ == kconst_float) {value_libary.push_back((unsigned long)root->f_val_); return "0.1";}
