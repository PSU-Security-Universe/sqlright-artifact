...
#include "../include/sqlite_norec.h"
...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  for (stage_cur = 0; stage_cur < 1; stage_cur++) {
    ...
    string result_str = read_sqlite_output_and_reset_output_file();
    if (result_str.find("Error") == string::npos){
      total_execs++;
    }
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    /* If the strip_sql is empty, do not save into the queue. */
    if (strip_sql.size() == 0) {
      ck_free(fn);
      return keeping;
    }

    add_to_queue(fn, strip_sql.size(), 0);
    ...
    fd = open(fn, O_WRONLY | O_CREAT | O_EXCL, 0600);
    if (fd < 0) PFATAL("Unable to create '%s'", fn);
    ck_write(fd, strip_sql.c_str(), strip_sql.size(), fn);
    close(fd);
...
keep_as_crash:
  ...
  /* if the hang or crash case is empty, do not save. */
  if (len == 0) {
    ck_free(fn);
    return 0;
  }
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  fprintf(plot_file,
          "%llu, %llu, %u, %u, %u, %u, %0.02f%%, %llu, %llu, %u, %0.02f, %u, %u, %u, %0.02f%%, %u, %u, %u, %u, %u, %u, %0.02f%%, %u, %u\n",
          ...
          bug_output_id, queued_with_cov); /* ignore errors */
  fflush(plot_file);
}
...
#define SP20    SP10 SP10
    ...
    */
    ...
    EXP_ST u8 common_fuzz_stuff(char **argv, string query_str) {
      ...
      queued_discovered +=
          save_if_interesting(argv, query_str.c_str(), query_str.size(), fault);
                             ...
                             int valid_max_num = 10)
    {
      ...
      int num_norec = 0;
      int max_trial =
          valid_max_num * 20; // For each norec select stmt, we have on average 3
      ...
      while (num_norec < valid_max_num)
      {
        ...
        // cerr << "new_norec_stmts: " << new_norec_stmts << endl;
        new_norec_stmts = g_mutator.validate(new_norec_stmts);
        if (new_norec_stmts == "") {
    ...
    /*
      ...
      */
      ...
      // ori_valid_stmts = {"SELECT COUNT (*) FROM x WHERE x;"};
      ...
      for(auto ir: mutated_tree){
        ...
        // }
        // cerr << "\n\n\nir_str: " << ir_str << endl;
        ir_str = g_mutator.validate(ir_str);
        ...
        if(common_fuzz_stuff(argv, ir_str)){
          goto abandon_entry;
        }
        stage_cur++;
        show_stats();
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QO:c:")) > 0)
