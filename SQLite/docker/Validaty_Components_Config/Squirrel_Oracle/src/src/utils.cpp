...
bool findStringInVector(vector<string> string_vector, string target) {
    // match the whole string in the vector, ignore cases.
    transform(target.begin(), target.end(), target.begin(), ::toupper);

    for (auto single_string : string_vector) {
        if (is_str_empty(single_string)){
            continue;
        }

        transform(single_string.begin(), single_string.end(), single_string.begin(), ::toupper);
...
void ensure_semicolon_at_query_end(string &stmt) {
  ...
  auto idx = stmt.end();
  idx--;
  for (; idx != stmt.begin(); idx--){
    if (*idx != ';' && *idx != ' '){
      break;
    }
  }
  stmt = stmt.substr(0, idx - stmt.begin()+1);
  stmt += ";";
}