...
// Postgres need to generate 
  vector<string> temp_valid_stmts = {
    ...
    // "SELECT * FROM x WHERE x=0 GROUP BY x;"   // The original Squirrel parser would encounter a parser error when rewrite the query to the TLP form with GROUP BY. Thus, ignore GROUP BY case. 
    // "SELECT * FROM x WHERE x=0 HAVING x;", // The original Squirrel parser doesn't support HAVING in its own. (Can combine with GROUP BY though)
    // "SELECT DISTINCT * FROM x WHERE x;", // The original Squirrel parser doesn't support DISTINCT
    // "SELECT MIN(x) FROM x WHERE x=0;",  // The original Squirrel parser doesn't support the TLP rewrite for aggregate function. 
    // "SELECT MAX(x) FROM x WHERE x=0;", // The original Squirrel parser doesn't support the TLP rewrite for aggregate function. 
    // "SELECT SUM(x) FROM x WHERE x=0;", // The original Squirrel parser doesn't support the TLP rewrite for aggregate function. 
    // "SELECT AVG(x) FROM x WHERE x=0;" // The original Squirrel parser doesn't support the TLP rewrite for aggregate function. 
  };
