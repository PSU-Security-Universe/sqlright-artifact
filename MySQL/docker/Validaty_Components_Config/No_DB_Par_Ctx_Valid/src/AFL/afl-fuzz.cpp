...
public:
  ...
  SQLSTATUS execute(const char *cmd, string& res_str)
  {
    ...
    for (string cur_cmd_str : v_cmd_str) {
      ...
      // cerr << "Executing command: " << cur_cmd_str << "\n\n\n";
      ...
      if ( is_oracle_select ) {
          if ( server_response == 0 ) {
            // cerr << "For statement: " << cur_cmd_str << "executed successfully \n\n\n";
            debug_good++;
          } else {
            // cerr << "For statement: " << cur_cmd_str << "executed failed \n\n\n";
            debug_error++;
          }
      }

      if (  cur_cmd_str.find("BEGIN VERI 0") != string::npos || cur_cmd_str.find("BEGIN VERI 1") != string::npos ) {  // The BEGIN VERI 0 will always be SELECT * FROM v0;
...
static void perform_dry_run(char **argv)
{
  ...
  while (q)
  {
    ...
    vector<IR *> ir_tree = run_parser(query_str);
                      ...
                      char **argv, u32 tmout = exec_tmout)
{
  ...
  for (string& cmd_string : cmd_string_vec) {
    ...
    /* 
    ** For the TLP oracle with Squirrel parser, we cannot implement UNION ALL statement into the parser due to parser conflicts. 
    ** At the same time, the Squirrel parser can ONLY support the basic type of the TLP statement, no GROUP BY, no aggregate function, no DISTINCT.
    ** Therefore, there is only one set operator possible for the TLP oracle with Squirrel parser, that is UNION ALL. 
    ** In the implementation, we directly use "UNION" to pass the parser, the mutator and the validate function, then we force change UNION to UNION ALL.  
    */
    if (cur_oralce_str == "TLP") {
      cmd_string = std::regex_replace(cmd_string, std::regex("UNION"), "UNION ALL");
    }
  }
  ...
  for (IR* ir_to_mutate : ori_ir_tree) {
    ...
    for (IR* mutated_ir_root : v_mutated_ir_root) {
      ...
      * consider appending anything */
      ...
      vector<IR *> cur_ir_tree = run_parser(ir_str);
      ...
      */
      ...
      if(!g_mutator.validate(cur_root)) {
        /* Empty */
      }

      p_oracle->ir_wrapper.set_ir_root(cur_root);
      vector<IR*> all_validated_stmts = p_oracle->ir_wrapper.get_stmt_ir_vec();

      vector<vector<vector<IR*>>> all_post_trans_vec_all_runs = g_mutator.post_fix_transform(all_validated_stmts, stmt_type_vec);
