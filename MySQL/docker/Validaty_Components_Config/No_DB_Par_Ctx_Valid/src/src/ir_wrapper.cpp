...
//     IR* cur_root = new IR(k, OP0(), NULL, NULL);
...
bool IRWrapper::is_in_subquery(IR* cur_stmt, IR* check_node, bool output_debug) {

    while (check_node->get_parent()) {
        if (check_node->get_ir_type() == kSelectNoParens) {
            if (check_node->get_parent() && check_node->get_parent()->get_ir_type() != kSelectStmt) {
                return true;
            }
        }
...
bool IRWrapper::is_exist_limit_clause(IR* cur_stmt){
    vector<IR*> v_limit_clause = get_ir_node_in_stmt_with_type(cur_stmt, kOptLimitClause, false);
...
bool IRWrapper::is_exist_set_operator(IR* cur_stmt) {
    ...
    vector<IR*> v_combine_clause = get_ir_node_in_stmt_with_type(cur_stmt, kCombineClause, false);
    if (v_combine_clause.size() > 0){
        return true;
    }
...
bool IRWrapper::is_exist_window_func_call (IR* cur_stmt) {
    vector<IR*> v_window_func_call = get_ir_node_in_stmt_with_type(cur_stmt, kWindowClause, false);
...
vector<IR*> IRWrapper::get_select_items_in_select_stmt(IR* cur_stmt){
    ...
    res_vec = this->get_ir_node_in_stmt_with_type(cur_stmt, kFunctionNameMark, false);
    return res_vec;
}
...
IR* IRWrapper::get_stmt_ir_from_stmtlist(IR* cur_stmtlist){
    ...
    if (
        ...
        cur_stmtlist->get_left()->get_ir_type() == kStmt &&
