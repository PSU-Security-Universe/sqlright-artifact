...
IR *IR::deep_copy() {
  ...
  copy_res = new IR(this->type_, op, left, right, this->float_val_,
                    this->str_val_, this->name_, this->mutated_times_, 0, kUse);
...
IR*  SelectClause::translate(vector<IR *> &v_ir_collector){
	...
	SWITCHSTART
		...
		auto tmp6= SAFETRANSLATE(opt_group_clause_);
		auto tmp7= SAFETRANSLATE(opt_window_clause_);

		res = new IR(kFunctionNameMark, OP3("SELECT", "(", ""), tmp1, tmp2);
		PUSH(res);
		...
		PUSH(res);
		res = new IR(kSelectClause, OP0(), res, tmp7);
		CASEEND
	SWITCHEND
...
IR*  CombineClause::translate(vector<IR *> &v_ir_collector){
	...
	SWITCHSTART
		...
		CASEEND
		CASESTART(3)
		res = new IR(kCombineClause, OP3("UNION ALL","",""));
		CASEEND
	SWITCHEND
...
IR*  IntLiteral::translate(vector<IR *> &v_ir_collector){
	TRANSLATESTART
		...
		res = new IR(kIntLiteral, string("0"), data_type_, scope_, data_flag_);
...
IR*  Identifier::translate(vector<IR *> &v_ir_collector){
	TRANSLATESTART
		...
		// cerr << "In ast.cpp, getting: " << string_val_ << "\n\n\n";
		res = new IR(kIdentifier, string(string_val_), data_type_, scope_, data_flag_);
