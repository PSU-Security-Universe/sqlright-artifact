...
vector<IR *> Mutator::mutate_all(IR *ori_ir_root, IR *ir_to_mutate, u64 &total_mutate_failed, u64 &total_mutate_num){
    ...
    if (ir_to_mutate->get_ir_type() == kProgram) {
...
#define MYSQLFUZZ
    ...
    while (getline(input_test, line)) {
        ...
        vector<IR *> v_ir = run_parser(line);
        ...
        v_ir = run_parser(line);
        // cerr << "Parsing line: " << line << "\n\n";
        if (v_ir.size() == 0) {
            cerr << "failed to parse after extract_struct: " << line << endl;
            continue;
        }
...
Mutator::fix_preprocessing(IR *stmt_root,
  ...
  set<DATATYPE> type_to_fix = {
    ...
    kDataDatabase
    // kDataLiteral, kDataIndexName, kDataDatabaseFollow, kDataTableNameFollow
...
bool Mutator::validate(IR * &root){
    ...
    /* The original squirrel code extract struct before parsing. */
    // extract_struct(root);

    reset_data_library();
    string sql = root->to_string();
    // IR* ast = parser(sql);
    vector<IR*> ir_res = run_parser(sql);
    // int ret = run_parser_multi_stmt(sql, ir_res);
...
int Mutator::try_fix(char* buf, int len, char* &new_buf, int &new_len){
    ...
    vector<IR*> ir_vec = run_parser(sql);
...
void Mutator::_extract_struct(IR *root) {
  ...
  if (root->get_ir_type() == kIntLiteral || root->get_ir_type() == kIntLiteral ) {
    if ( root->str_val_ != "") {
      root->int_val_ = 0;
      root->str_val_ = "0";
      return;
  ...
  if (root->data_type_ != kDataFunctionName && root->str_val_ != "" && root->data_flag_ == kDefine) {
...
void Mutator::add_all_to_library(string whole_query_str,
  ...
  for (auto current_query : queries_vector) {
    ...
    vector<IR *> ir_set = run_parser(current_query);
...
void Mutator::add_to_library(IR *ir, string &query) {
  ...
  if (ir_libary_2D_hash_[p_type].find(p_hash) !=
      ir_libary_2D_hash_[p_type].end() && p_type == kProgram) {
...
IR *Mutator::get_from_libary_with_type(IRTYPE type_) {
  ...
  */
  ...
  if (all_matching_node.size() > 0) {
    ...
    current_ir_set = run_parser(*p_current_query_str);
...
bool Mutator::add_missing_create_table_stmt(IR* ir_root) {
  ...
  if (ir_root->get_ir_type() != kProgram) {
    return false;
  }
